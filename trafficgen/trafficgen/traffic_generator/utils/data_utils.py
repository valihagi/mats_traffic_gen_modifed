...
def process_case_to_input(case, agent_range=60, max_agent_num=32):
    inp = {}
    ...
    agent = agent[:max_agent_num]
    mask = np.ones(agent.shape[0])
    mask = mask[:max_agent_num]
    agent = np.pad(agent, ([0, max_agent_num - agent.shape[0]], [0, 0]))
    mask = np.pad(mask, ([0, max_agent_num - mask.shape[0]]))
    inp['agent'] = agent
...
def get_vec_based_rep(case_info, max_agent_num=32):
    thres = 5
    ...
    total_mask = agent_mask #min_dist_mask * agent_mask * v_dir_mask * dir_mask
    total_mask[:, 0] = 1
...
def process_data_to_internal_format(data, max_agent_num=32):
    case_info = {}
    gap = 20
    ...
    get_vec_based_rep(case_info, max_agent_num=max_agent_num)
