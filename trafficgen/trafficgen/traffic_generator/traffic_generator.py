...
class TrafficGen:
    ...
    def inference_control(self, data, ego_gt=True, context_num=1, length=190, per_time=20):
        ...
        if ego_gt == True or context_num > 0:
            future_traj = data['gt_agent']
            pred_agent[:, :context_num, :7] = future_traj[:length, :context_num]
            start_idx = context_num
        ...
        for i in range(0, length - 1, per_time):
            ...
            for case in case_list:
                inp_list.append(process_case_to_input(case, max_agent_num=32))
