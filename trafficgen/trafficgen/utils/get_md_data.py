...
def _extract_map(map_feat, sample_num):
    ...
    for map_feat_id, map_feat in map_feat.items():
        ...
        if "polygon" in map_feat:
            map_feat['polyline'] = map_feat['polygon']
...
def metadrive_scenario_to_init_data(scenario):
    ...
    for step in range(track_len):
        ...
        for traffic_light_index, traffic_light in traffic_lights.items():
            ...
            state = traffic_light_state["object_state"]
            if state is None:
                traffic_light_step_data[4] = 0
                traffic_light_step_data[5] = 0
            else:
                if state == "LANE_STATE_UNKNOWN" or state is None:
                    state = 0
                elif state == "LANE_STATE_STOP" or state == "LANE_STATE_ARROW_STOP":
                    state = 1
                elif state == "LANE_STATE_CAUTION" or state == "LANE_STATE_ARROW_CAUTION":
                    state = 2
                elif state == "LANE_STATE_GO" or state == "LANE_STATE_ARROW_GO":
                    state = 3
                else:
                    raise ValueError("Unknown traffic light state: ", state)

                traffic_light_step_data[4] = state
                traffic_light_step_data[5] = 1
